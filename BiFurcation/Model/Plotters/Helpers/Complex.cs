...
namespace BiFurcation {
  public class Complex {
    ...
    public static Complex power(Complex C, double p) {
      double theta = Math.Atan(C.Im / C.Re);
      double radius2 = Math.Pow(C.MagnitudeSquared, p / 2);
      return new Complex(radius2 * Math.Cos(p * theta), radius2 * Math.Sin(p * theta));
    }
